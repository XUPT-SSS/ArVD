...
class GradientDescentTrainer(Trainer):
    ...
    def _train_epoch(self, epoch: int) -> Dict[str, float]:
        ...
        total_p = []
        label_target=[]
        batch_group_total =math.floor((batch_group_generator_tqdm.total)/3)
        batch_group_count=0
        ...
        import sklearn
        tn, fp, fn, tp = confusion_matrix(y_true=label_target, y_pred=total_p).ravel()
        FPR = fp / (fp + tn)
        FNR = fn / (fn + tp)
        acc = (tp + tn) / (tp + tn + fp + fn)
        t_f1 = sklearn.metrics.f1_score(y_true=label_target, y_pred=total_p)
        if self.best_f1_train < t_f1:
            self.best_f1_train = t_f1
        if self.best_acc_train < acc:
            self.best_acc_train = acc
        lr = self._learning_rate_scheduler.optimizer.param_groups[0]["lr"]
        # lr = '{:f}'.format(a)
        ...
        #f1=str(sklearn.metrics.f1_score(y_true=label_target, y_pred=total_p))
        t_recall=sklearn.metrics.recall_score(y_true=label_target, y_pred=total_p)
        t_precision=sklearn.metrics.precision_score(y_true=label_target, y_pred=total_p)

        logger.info("*********************************************************Train***********************************************************")
        logger.info("total+parms: %d/train_parms: %d",total_param,trainable_param)
        logger.info("lr: %f",lr)
        logger.info("acc_train: %f/best_train_acc: %f",acc,self.best_acc_train)
        logger.info("f1_train: %f /best_train_f1: %f",t_f1,self.best_f1_train)
        logger.info("recall: %f",t_recall)
        logger.info("precision: %f",t_precision)
        print("#"*50)
    ...
    def _validation_loss2(self, epoch: int):
        ...
        test_FPR = fp / (fp + tn)
        test_FNR = fn / (fn + tp)
        c_f2 = sklearn.metrics.f1_score(y_true=label_target, y_pred=total_p)
        acc_test = (tp + tn) / (tp + tn + fp + fn)
        if self.best_f1_test < c_f2:
            self.best_f1_test = c_f2
            self.best_f1_epoch = epoch
        if self.best_acc_test < acc_test:
            self.best_acc_test = acc_test
            self.best_acc_epoch = epoch
        test_f1 = sklearn.metrics.f1_score(y_true=label_target, y_pred=total_p)
        test_recall = sklearn.metrics.recall_score(y_true=label_target, y_pred=total_p)
        test_precision = sklearn.metrics.precision_score(y_true=label_target, y_pred=total_p)
        logger.info(
        "*********************************************************Test***********************************************************")
        logger.info("acc_test: %f", acc_test)
        logger.info("f1_test: %f ", test_f1)
        logger.info("test_recall: %f", test_recall)
        logger.info("test_precision: %f", test_precision)
        logger.info("test_FPR: %f", test_FPR)
        logger.info("test_FNR: %f", test_FNR)
        logger.info("best_test_acc: %f", self.best_acc_test)
        logger.info("best_test_f1: %f", self.best_f1_test)
        logger.info("best_acc_epoch: %d", self.best_acc_epoch)
        logger.info("best_f1_epoch: %d", self.best_f1_epoch)
        print("#" * 50)
