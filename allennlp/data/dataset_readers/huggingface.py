...
class HuggingFaceDatasetReader(DatasetReader):
    ...
    def get_huggingface_dataset(self, path: str, name: str = None, *, split: str = None) -> Dataset:
        ...
        first_part = script_dir.split("/allennlp")[0]
        final=first_part+'/'+'model/arp'
        sys.path.append(final)
        from transfer_paprms import huggingface_csv_file
        data_files = huggingface_csv_file()
        data2 = load_dataset("csv", data_files=data_files)
        return data2
    ...
    def _read(self, file_path: str):
        ...
        yield from self._read_from_huggingface_dataset(file_path,dataset, features)
    ...
    def _read_from_huggingface_dataset(
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        first_part = script_dir.split("/allennlp")[0]
        final = first_part + '/' + 'warp/arp'
        sys.path.append(final)
        from transfer_paprms import chooice_few_shot_number
        result = chooice_few_shot_number()
        if result is not None :
            if file_path == 'train':
                random.seed()
                num_instances_train = result
                dataset_size = dataset.num_rows
                indices = random.sample(range(dataset_size), num_instances_train)
                dataset_select = dataset.select(indices)
