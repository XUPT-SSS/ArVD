...
from allennlp.training.metrics import CategoricalAccuracy, Metric,F1Measure,FBetaMeasure
...
class ArpClassifier(Model):
    ...
    def forward(  # type: ignore
        ...
        if label is not None:
            if self._num_labels > 1:
                ...
                #output_dict["accuracy"] = self._accuracy.get_metric()
                # Shape: (batch_size,)
                ...
                loss = self._loss(logits.squeeze(-1), label_tensor)
                output_dict["loss"] = loss
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        for metric in self._metrics:
            # result =metric.get_metric(reset)
